...
config.notifyByEmail = envVars.filter(k => process.env[k] && (process.env[k].trim().length > 0) ).length === envVars.length;
...
const notify = (domain) => {
  if( config.notifyByEmail ) {
    ...
    sendMail({
      ...
      html: config.email.htmlData.map(l => l.replace(wildcard, domain)).join("<br>")
    }, (err, res) => {
      err ? 
